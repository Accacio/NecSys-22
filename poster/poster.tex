...
\begin{frame}
  \begin{textblock}{15.6}(.0,-5.)
    \begin{block}{1. Context - False Data injection in dMPC exchange}
      ...
      \begin{minipage}[c]{18cm}
        ...
        \end{equation*}
        \vskip 1cm
      % {\color{ietr_brightblue} \bfseries HARD TO COMPUTE}
      {\color{ietr_brightblue} \bfseries HARD TO COMPUTE}
      \end{minipage}
      \begin{minipage}[c]{3cm}
        \tikz \draw [line width=7pt, arrows = {-Latex}] (0,0) -- (2,0);
      ...
      \begin{minipage}[c]{35cm}
        % \centering Primal decomposition dMPC
        % \vskip -1cm
        \begin{figure}[h]
          \resizebox{35cm}{!}{
          \begin{tikzpicture}[thick,node distance=5.cm and 1cm,
            ...
            \node[draw,
            coordinator,
            ] (coordinator) at ($(mult) + (-4,-8)$) {};
            \node[align=center] at ($(coordinator)+(.75,0)$) {
              \centering
              \begin{minipage}{25cm}
    ...
    \begin{center}
      \begin{minipage}[c]{.95\textwidth}
        ...
        \begin{tikzpicture}
          ...
          \draw[line width=.5cm,red,latex-] (10.5,-10) -- (9,-5.5) node[above] {Unstable};
          \node[draw,circle,minimum width=2cm,line width=.2cm,blue] (circle) at (-4.8,-12.) {};
        ...
        \end{tikzpicture}
         ...
         \\[1ex]
         {What if we estimate $T_{i}[k]$ and invert it?}
         \\[1ex]
  ...
  \begin{textblock}{5.1}(\seccol,\secrow)
    \begin{block}{3. Estimating cheating matrix $T_{i}[k]$}
      \begin{center}
          \centering
          \begin{minipage}[c]{.95\textwidth}
            Local pbs.~\eqref{eq:local_problem} are {\bf QP} $\to$ {\bf Explicit Solution}
            \\{\bf PWA form w.r.t $\vec{\theta}_{i}$}:
            \begin{equation}
              \centering
            ...
            \begin{itemize}
              \item Classify data in regions
              \item Estimates parameters using weights
            \end{itemize}
            \vspace{1ex}
            EM estimates $\tilde{P}_{i}^{1}[k]$ if we provide minimally excited inputs $\vec{\theta}_{i}$ and $\tilde{\vec{\lambda}}_{i}$.
          \end{minipage}
          \begin{itemize}
            \item Minimally excited inputs
                  % \todo{artificial scarcity sampling}
                  \begin{itemize}
...
\begin{minipage}[c]{0.95\linewidth}
          ...
          \begin{algorithm2e}[h]
            ...
            \Repeat{$\set{P}_{\mathrm{cur}}$ converges to local maximum}{
