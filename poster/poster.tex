    ...
    \begin{block}{1. Challenge - False Data injection in dMPC exchange}
      \begin{minipage}[c]{0.31\paperwidth}
        ...
        \begin{itemize}
          ...
          \item Coupling constraint {\large $ \sum_{i=1}^{M}\Gamma_{i}\vec{U}_{i}\leq\vec{U}_{\max}$}
        \end{itemize}
        % \vspace{.25cm}
        \resizebox{!}{8cm}{
              ...
              \begin{minipage}{22cm}
                \begin{equation}
                  \begin{matrix}
                    ...
                    \mathrm{s.t.}&&&  \bar{\Gamma}_{{\rm I}}\vec{U}_{{\rm I}}[k]\preceq\vec{\theta}_{{\rm I}}[k]:\vec{\lambda}_{{\rm I}}\\
                    &&&\vec{U}_{{\rm I}}[k]\succeq\0
...
\eqref{eq:local_problem}
            ...
            \draw[-latex,line width=9pt] (block1.south)+(1,.0) -- ( coordinator.north -| {$(block1.south)+(1,.0)$}) node [right,midway] {$\vec{\lambda}_{1}[k]$};
            \draw[latex-,line width=9pt] (block1.south)+(-1,0) -- (  coordinator.north -| {$(block1.south)+(-1,0)$}) node [left,midway] {$\vec{\theta}_{1}[k]$};

            \draw[-latex,line width=9pt] (blockM.south)+(1,.0) -- ( coordinator.north -| {$(blockM.south)+(1,.0)$}) node [right,midway] {$\vec{\lambda}_{M}[k]$};
  ...
  \begin{block}{2. Attack and consequences}
    \begin{center}
      \begin{minipage}[c]{.95\textwidth}
        ...
        Attacker chooses an \emph{invertible linear function}
        ...
        % \end{assumption}
        \textbf{\textit{Remark}}\\
        {\color{ietr_brightblue} Attacker says it is satisfied  \underline{\bf only if it is}}
        \begin{center}
          \begin{tikzpicture}
            \node (effects) at (0,0) {
...
\begin{minipage}[c]{0.95\linewidth}
  ...
  \begin{textblock}{15.6}(.0,3.9)
    \begin{block}{6. Example: distributed control for a heating network under power scarcity }
      \small
      ...
      \end{minipage}
          ...
          \node[align=center,red] (attack) at (0,-6.) {\large \faFlash\\\large Attack };
          \draw[line width=.275cm,red,latex-] (-12.,-1.2) -- (attack.west) ;
          \draw[line width=.275cm,red,latex-] (14.5,-4) -- (attack);
        \end{tikzpicture}
      \end{minipage}
    \end{block}
  \end{textblock}
