...
\end{equation}
where \todo{$\Gamma_{i}:\semidefpos^{n_{u}}$}.
...
\todo{Reference tracking can be written as ${\vik=\wik-C_{i}\xik}$, where $\wik$ is a state reference.
Disturbance rejection can be written as ${\vik=C_{i}\xik}$.
}
...
\todo{The sum $\sum_{i=1}^{M}\vec{\theta}_{i}$ can also be represented by the matrix multiplication $I_{c}^{M}\vec{\theta}$.
}
...
where ${I_{c}^{M}=\kron{\1_{M,1}}{I_{c}}}$, with ${c=N_{p}\pi_{\uik}}$.
Observe that ${I_{c}^{M} \pseudoinv{I_{c}^{M}}\vec{v}=\frac{\sum_{i=0}^{M}\vec{v}_{i}}{M}}$.
...
\todo{
  Observe that $H_{i}:\defpos$, and that $\fik$ varies with $k$.
}
