...
\todo{Voilà ce que je propose en introduction. Il manque les références et
peut-être un ou deux articles (j'ai laissé un endroit « libre »). Redis moi
ce que tu en penses.
}
...
the $M$ agents are coupled by a global input constraint with weighting matrices \todo{${\Gamma_{i}:\semidefpos^{n_{u}}}$}:
\begin{equation}
...
\subsection{Primal Decomposition based \todo{dMPC}}\label{ssec:dMPC}
...
\todo{
  Since $\bar{\Gamma}_{i}$ are block diagonal we can assure\pdfnote{proof?} there exists one zone where all constraints are active.
}
\begin{assumption}
  We suppose that for every time $k$ there exists a zone for each agent $i$ where all constraints are active.
\end{assumption}

Fixing the index for the zones where all constraints are active to $1$, we can called them the $1$-zones.
We can estimate the parameters of their respective solution in~\eqref{eq:linear_cheating} with
\begin{equation}
  \label{eq:lambdafuntheta_tilde}
\lambdaicheat[k]=\gamma_{i}(\lambdai(\thetaik))=-\widehat{\tilde{P}_{i}^{1}}[k]\thetaik-\widehat{\tilde{\vec{s}}_{i}^{1}}[k].
\end{equation}
