...
\begin{abstract}% Abstract of not more than 250 words.
  ...
  Communication among agents is crucial to achieve consensus and optimal global behavior.
  ...
  The performance is assessed through academical example of the temperature control of multiple rooms under input power constraints.
\end{abstract}
...
Given the complexity, scale and even geographic location of these structures, it is necessary to adopt control structures that are decentralized rather than monolithic.
...
This approach can be used, i.e., for the voltage regulation of electrical networks, or the production-consumption balance in these same networks, water distribution and many other applications.
...
A previous work~\citep{NogueiraEtAl2021} proposed a safe algorithm based on data reconstruction to mitigate the effects of a \fdi{} in the communication between a coordinator and local constraint-free agents, which were bound by global equality constraints.
...
First, in Section~\ref{sec:PS}, we introduce the primal decomposition-based \dmpc{} and its vulnerabilities.
...
Moreover, in Section~\ref{sec:App_dMPC}, we illustrate the algorithm with academic simulation and we assess its performance.
...
\end{equation}
with ${\umax:\R^{n_{u}}}$.
The agents are coupled by global input constraints with weighting matrices \todo{${\Gamma_{i}:\semidefpos^{n_{u}}}$}:
\begin{equation}
...
Solving~\eqref{eq:GOP} at each time $k$ can be quite challenging, since the prediction horizon and the number of agents can be large making the process computationally intensive. Another issue that can arise is the fact that the complete information \todo[verify]{(${\set{I}_{i}=\{A_{i},B_{i},C_{i},\vec{v}_{i}[k],Q_{i},R_{i}\}}$)} from all the subsystems is needed to solve the problem,
...
\todo[make a better figure if time permits]{
\begin{figure}[t]
  \centering
  \begin{tikzpicture}[node distance=1.5cm]
    \tikzset{
      agent/.style={draw,circle,minimum width=1cm,},
      coord/.style={draw,circle,minimum width=1cm,very thick}
    }
    ...
    \node[coord] (c) [above of=a2] {C};


    \draw[-latex] (a1) to [out=60,in=210] node [sloped,above] {$\vec{\lambda_{1}}[k]$} (c) ;
    \draw[-latex] (c) to [out=-120,in=30] node [sloped,below] {$\vec{\theta}_{1}[k]$} (a1);

    \draw[-latex] (am) to [out=150,in=-60]node [sloped,below] {$\vec{\lambda}_{M}[k]$} (c);
    \draw[-latex] (c) to [out=-30,in=120] node [sloped,above] {$\vec{\theta}_{M}[k]$} (am);

  \end{tikzpicture}
  \caption{Exchange between coordinator (C) and agents $a_{i}$.}\label{fig:dmpc_graph}
\end{figure}
}
...
The systems are modeled as continuous-time linear \mbox{time-invariant} systems using the \mbox{3R-2C} model~\citep{NogueiraEtAl2021}, with parameters shown in Tables~\ref{tab:modelParamMeaning} and~\ref{tab:modelParam}, and dynamics
\begin{equation}
\begin{matrix}
  \label{eq:systems}
...
\resizebox{.98\columnwidth}{!}{$
  ...
  \end{matrix},
\end{equation*}
where ${\vec{x}_{i}=[{{x}_{A}}_{i}\T\ {{x}_{W}}_{i}\T]\T}$, which represents the mean temperatures of the air~(${x_A}_i$) and walls inside room~$i$~(${x_W}_i$), and $\vec{u}_{i}$ is the input (the heating power)
...
performances of our proposition by comparing the inputs in Fig.~\ref{fig:control_3Scenarios}. When room~I is selfish, its control increases while the other decreases, unless the correction is activated and it approaches the nominal.
...
The first phase of the algorithm consists of identifying the attacker by exploiting the exchange structure.
...
This principle should be generalized to cases when the attack is not completely invertible, reconstructing by parts the original mechanism.
Also, other types of decomposition structures and attack models need to be explored, and this is what we plan to do in the near future.
